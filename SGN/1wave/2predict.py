...
with np.load(save_path, allow_pickle=True) as data:
    ...
    # X_feat, Y_feat = tool.bulid_feat(data)
    X_feat, Y_feat = tool.build_feat_SG(data,7,2)
...
for i in range(len(X_feat)):
    if i == 0:
        ...
        xy_t_current = X_feat[i][:, :5].clone()  # First 5 dimensions (e.g., x, y, t, derivatives) (Nx*Ny, 5)

        _input_feat = torch.cat([xy_t_current, prev_pred], dim=1)  # Concatenate predicted u_prev
...
# Calculate error: Align shapes of the true solution and the predicted solution
