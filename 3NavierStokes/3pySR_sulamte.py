...
def bulid_aggr_feat(result,data_node):
    ...
    需要从result中构造出'values_x,values_y,x,y,t,ux, uy,uxx,uyy, x_pred, y_pred]
    然后将x,y,t相等的项合并为feat[values_x,values_y,x,y,t,e1,e2,e3,e4,ux, uy, x_pred, y_pred]
    注意：以data_node中x,y,t为主，即len(feat)=len(data_node)
    feat: (len(data_node), 7+6) -> [values_x,values_y,x, y, t, ux, uy, uxx, uyy, x_pred,y_pred,e1,e2,e3,e4]  # 排除初始步，只取从 i=1 开始
    """
    ...
    for i in range(1, original_Nt_plus1):
        ...
        # 计算 ux = ∂(x_solution(t-1))/∂x 和 uy = ∂(y_solution(t-1))/∂y
        ux_2d = savgol_filter(x_prev_2d, 7, 2,deriv=1,delta=dx,axis=1)  # 平滑处理
        uy_2d = savgol_filter(y_prev_2d, 7, 2,deriv=1,delta=dy,axis=0)  # 平滑处理

        # 计算二阶偏导 uxx = ∂²(x_solution(t-1))/∂x² 和 uyy = ∂²(y_solution(t-1))/∂y²
        uxx_2d = savgol_filter(x_prev_2d, 7, 2,deriv=2,delta=dx,axis=1)  # 平滑处理
        uyy_2d = savgol_filter(y_prev_2d, 7, 2,deriv=2,delta=dy,axis=0)  # 平滑处理

        feat_step = pd.DataFrame({
            'x': X.flatten(),
            'y': Y.flatten(),
            't': np.full(X.size, np.round(t, 6)),  # 时间取当前步
            'values_x': values_x[i].flatten(),  # 当前 x_solution
            'values_y': values_y[i].flatten(),  # 当前 y_solution
            'ux': ux_2d.flatten(),
            'uy': uy_2d.flatten(),
            'uxx': uxx_2d.flatten(),
            'uyy': uyy_2d.flatten(),
            'x_prev': x_prev_2d.flatten(),  # 前一步 x_solution
            'y_prev': y_prev_2d.flatten()   # 前一步 y_solution
        })
...
data_msg_1 = data_msg_1.drop_duplicates() #去重
...
print(f'聚合特征：{data_node_y.shape}')
...
# msg1待选：  e0,  dx, dy, t, x_prev1,  y_prev1,  x_prev2,  y_prev2,  ux1,  ux2,  uy1,  uy2, uxx1, uxx2, uyy1, uyy2
...
# msg2待选：  e1,  dx, dy, t, x_prev1,  y_prev1,  x_prev2,  y_prev2,  ux1,  ux2,  uy1,  uy2, uxx1, uxx2, uyy1, uyy2
...
# nodeX待选： values_x, x,  y,  t,  ux, uy, uxx, uyy, x_prev, e0, e1
node_x_feature_names = ['x', 'y', 't', 'ux','uy', 'x_prev', 'e0', 'e1']
# nodeY待选： values_y, x,  y,  t,  ux, uy, uxx, uyy, y_prev, e0, e1
...
print(f"最佳消息聚合表达式x:{SR_node_x.sympy()},损失为:{loss_node_x}")
formula_node_y = SR_node_y.get_best()
loss_node_y= formula_node_y['loss']
print(f"最佳消息聚合表达式y:{SR_node_y.sympy()},损失为:{loss_node_y}")
